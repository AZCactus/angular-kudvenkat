angular.module('myModel', []).controller("myController", function($scope){
  ...
  $scope.getSearchObj = function() {
    console.log("geting search obj");
    var searchobj = {};
    if($scope.searchText) {
      if($scope.searchText.first_name)
        searchobj.first_name = $scope.searchText.first_name;
      if($scope.searchText.email)
        searchobj.email = $scope.searchText.email;
    }
    return searchobj;
  }
})
